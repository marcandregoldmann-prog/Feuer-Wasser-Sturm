...
const App = (() => {
  ...
  function togglePause() {
    ...
    if (gamePaused) {
      ...
      // Resume wird automatisch durch Loop fortgesetzt
    }
  }
  ...
  function setDifficulty(level) {
    difficulty = level;
    localStorage.setItem('fws_difficulty', level);
    updateDifficultyUI();
  }
  ...
  function toddlerLoop() {
    ...
    // Timing based on difficulty: 12–20 seconds (base)
    const mult = getTimingMultiplier();
    const waitSeconds = rand(12 * mult, 20 * mult);
    ...
    const t = setTimeout(() => {
      ...
      // Reaction countdown for toddlers: 8 seconds (base)
      const reactionTime = 8 * mult;
  ...
  function chaosLoop() {
    ...
    // Timing based on difficulty: 3–12 seconds (base)
    const mult = getTimingMultiplier();
    const waitSeconds = rand(3 * mult, 12 * mult);
    const waitMs = waitSeconds * 1000;
  ...
  function chaosCommandSequence(total, current) {
    ...
    const cmd = pickCommand();
    // Reaction time based on difficulty: 2–4 seconds (base)
    const mult = getTimingMultiplier();
    const reactionTime = rand(2 * mult, 4 * mult);
    showCommand(cmd);
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Load sound settings
  const savedSound = localStorage.getItem('fws_sound');
  if (savedSound === '0') {
    App.toggleSound();
  }
